...
if [ "$1" == "commit" ]; then
    ...
    IFS=$'\n\n' read -ra commit_message_parts <<< "${commit_message_full}"
    commit_message_subject=$(echo "${commit_message_parts[0]}" | sed 's/^Subject: //')
    commit_message_body=$(echo "${commit_message_parts[1]}" | sed 's/^Description: //' | sed 's/\\n/\n/g')
